<x-app-layout>
    ...
    <div class="py-4">
        <table class="w-full bg-[#222222] text-neutral-300 rounded-md shadow-md ">
            ...
            <tbody>
                @foreach ($vouchLogs as $log)
                    <tr class="hover:bg-neutral-800">
                        <td class="px-4 py-2 border-b border-neutral-800">{{ $log->created_at->format('d-m-Y H:i') }}
                        </td>
                        <td class="px-4 py-2 border-b border-neutral-800" style="color: {{ $log->vouchedByUser->getUsernameColor() }};">
                            <a href="{{ route('profile.show', $log->vouchedByUser) }}">
                                {{ $log->vouchedByUser->username }}
                            </a>
                        </td>
                        <td class="px-4 py-2 border-b border-neutral-800" style="color: {{ $log->user->getUsernameColor() }};">
                            <a href="{{ route('profile.show', $log->user) }}">
                                {{ $log->user->username }}
                            </a>
                        </td>
                        <td class="px-4 py-2 border-b border-neutral-800">{{ $log->reason }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
